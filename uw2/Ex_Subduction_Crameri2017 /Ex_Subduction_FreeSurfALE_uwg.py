...
ymin, ymax = ndim(-800 * u.kilometer), ndim(0 * u.kilometer)
yint = 0.

boxHalf = (xmax-xmin)*0.5
#hpc
# dy = ndim(2.5 * u.kilometer)
# dx = ndim(2.5 * u.kilometer)
...
use_fssa = False
if use_fssa:
    outputPath = "op_Ex_Subduction_FreeSurf_ALE_withFSSA0.5_yres{:n}_uwg".format(yRes)
else:
    outputPath = "op_Ex_Subduction_FreeSurf_ALE_noFSSA_yres{:n}_uwg".format(yRes)
...
Model.set_temperatureBCs(top=refTempSurf) 
