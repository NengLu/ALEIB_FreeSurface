...
use_fssa = False
if use_fssa:
    outputPath = "op_Ex_2DTopoRelax_FreeSurf_ALEIB_withFSSA0.5_yres{:n}_tRatio{:n}_Swarm_uwg".format(yRes,tRatio)
    save_every = 1
else:
    outputPath = "op_Ex_2DTopoRelax_FreeSurf_ALEIB_noFSSA_yres{:n}_tRatio{:n}_Swarm_uwg".format(yRes,tRatio)
    save_every = 5
...
#     Fig = vis.Figure(resolution=(500,500),rulers=True,margin = 100)
#     Fig.Mesh(Model.mesh)
#     Fig.show()
...
conditions = uw.conditions.DirichletCondition(variable=TField,indexSetsPerDof=(top + bottom+Sets_ib,))
...
TField.data[axis_ib, 0] = perturbation(init_mesh.data[axis_ib,0])  
